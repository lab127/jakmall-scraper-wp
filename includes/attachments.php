...
function insert_product_attachment ($author, $imagepaths, $post_id, $filealt="images", $post_date=null) {

    global $wpdb;

    $attids = [];
    for ($i=0; $i < count($imagepaths); $i++) { 
        $tmp_upload = $imagepaths[$i];
        // $cek_width = getimagesize($tmp_upload);
        $filename = pathinfo($tmp_upload, PATHINFO_FILENAME); // filename
        $basename = pathinfo($tmp_upload, PATHINFO_BASENAME); // basename.ext

        $source = $tmp_upload; // This is going to be where the image is located, depending on the fileloc you pass in this may not be needed
    ...
    return $attids; // Return the images id to use in the below functions
}
